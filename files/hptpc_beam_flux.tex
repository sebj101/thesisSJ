...
As mentioned in \citesec{sec:theory:currentState}, the targets of the next generation of long baseline neutrino oscillation experiments include the measurement of non-zero \dcp and the neutrino mass hierarchy.
...
The T2K experiment has reports systematic uncertainties of $7-9\%$ on the rate of far detector electron-like events (after the near detector constraint has been applied) where cross-section uncertainties are the largest part of this~\cite{t2kRecent}.
...
Given that in a detector only the outgoing particles are observed, we are reliant on neutrino interaction models of the initial nuclear state, nuclear effects and FSI in order to correctly reconstruct the neutrino energy.
...
This wss in an effort to make a measurement of the proton-argon cross-section at low energies (proton kinetic energies of less than \SI{50}{\mega\electronvolt}).
...
From previous GEANT~\cite{geant} simulations it is determined that the optimal off-axis angle for the HPTPC is between \ang{2} and \ang{3}.
...
The distances to the various objects in the beamline from an origin point are measured by the CERN Survey, Mechatronics and Measurements (SMM) group to a precision of \SI{0.5}{\mm}.
...
\begin{figure}[h]
  ...
  \end{adjustbox}
  \caption[Angular distribution of objects in the T10 beamline]{Angular distribution of objects within the T10 beamline. US and DS refer to the upstream and downstream faces of the TPC respectively. The origin here is at \SOne.}
  \label{fig:angDistS1}
\end{figure}
...
\citefig{fig:beamlinePics} shows images of the beamline with the various timing points as well as the HPTPC and moderator blocks labelled.
...
A diagram of the HPTPC, displaying a cross-sectional view of the active volume, is shown in \citefig{fig:hptpc}.
...
In a secondary beamline such as the T10 beamline at CERN, charged particles are selected by momentum.
...
At the momenta and within the beamline used in this study, MIPs are primarily charged pions and electrons.
...
This is caused by deuterons which are attenuated in the other samples.
...
\begin{figure}[t]
  ...
  \caption[Comparison of $\SOne-\STwo$ hits in the two ToF DAQs]{Comparison of $\SOne-\STwo$ hits in the two ToF DAQs. A first-order polynomial (red) is fitted in each case.}
...
This is due to protons losing energy in the walls of the HPTPC pressure vessel which in lead to a slower time of flight and therefore a shifted peak.
...
One can clearly see that the energy range of protons is changed from \SIrange{210}{320}{\mega\electronvolt} for the unmoderated beam to \SIrange{60}{110}{\mega\electronvolt} when four moderator blocks are placed in the beamline.
