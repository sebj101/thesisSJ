...
\begin{figure}[h]
  ...
  \caption[Images of the test beam setup with various objects labelled]{Images of the T10 beamline with the apparatus used in the beam test labelled. Left: the downstream part of the beamline. Right: The upstream part of the beamline.}
...
\begin{figure}[h]
  ...
  \caption[The upstream beam counters \SOne and \STwo]{Images of the upstream beam counters \SOne (left) and \STwo (right)}
  \label{fig:s1s2Pic}
\end{figure}
...
An example of the distribution produced is shown in \citefig{fig:s1Res}.

\begin{figure}[h]
  \begin{adjustbox}{max totalsize=.7\textwidth, center}
    \input{files/figures/hptpc_beam_flux/s1TimeRes}
  \end{adjustbox}
  \caption[Example of \SOne time resolution measurement]{An example of an \SOne time resolution measurement made using the quantity $t_{\text{ave}}$.}
  \label{fig:s1Res}
\end{figure}
