...
The T2K experiment reports systematic uncertainties of $7-9\%$ on the rate of far detector electron-like events (after the near detector constraint has been applied) where neutrino interaction uncertainties are the largest part of this~\cite{t2kRecent}.
...
This is shown in \citefig{fig:protonNucleus} which compares proton cross-section data with a semi-empirical model.
...
\citefig{fig:generatorProtons} also shows that much of the discrepancy lies below the proton track reconstruction threshold in liquid argon.
...
An overview of the beam test, including the experimental setup is given in \citesec{sec:hptpc_beam_flux:overview} while \citesec{sec:hptpc_beam_flux:methods} covers the methods used in the analysis. 
Measurements of the beam flux are presented in \citesec{sec:hptpc_beam_flux:results} along with comparisons with simulations.
These simulations were designed and run by a fellow HPTPC collaborator, Toby Nonnenmacher.
However, the analysis of these simulations is my own work.
...
\citefigL{fig:unmoderatedBeam}, shows the time of flight spectrum for the unmoderated T10 beam at a momentum setting of \SI{0.8}{\giga\electronvolt\per\clight}.
In this case the baseline is \SI{10.8}{\metre}.
Within \citefigL{fig:unmoderatedBeam}, there are several peaks caused by different particle species, including protons.
The particle identification process will be covered in more detail in \citesec{sec:hptpc_beam_flux:methods}.
One can see from \citefigR{fig:unmoderatedBeam}, that even at this momentum setting there is a negligible flux of protons with kinetic energies of less than \SI{0.1}{\giga\electronvolt}.
...
A bird's-eye view diagram of the experimental setup, which shows the beam position relative to the HPTPC, is shown in \citefig{fig:beamSetup}.
...
These timing points are labelled in \citefig{fig:beamSetup} and, from upstream to downstream, consist of:
\begin{itemize}
    ...
    \item \SFour, a large panel of plastic scintillator bars placed directly downstream of the HPTPC, which is described in \citesec{sec:hptpc_beam_flux:overview:s4} (its characterisation is described in \citesec{sec:hptpc_dtof_characterisation}).
\end{itemize}
...
From these measurements, the distance between various timing points is calculated and displayed in \citetab{tab:pointDistances}.
...
\citetab{tab:angDistS1} shows the angular extent of various beam line components as measured from \SOne.
\citefig{fig:angDistS1} shows the angular distribution of objects within the beam line as viewed from \SOne.
...
\citefig{fig:beamlinePics} shows images of the beam line with the various timing points as well as the HPTPC and moderator blocks labelled.
...
Two separate DAQs were used since the two systems were constructed at different locations, independently of one another.
...
\begin{figure}[h]
  \centering
  \includegraphics[width=.7\linewidth]{files/figures/hptpc_beam_flux/S1S2FrontOn.pdf}
...
The time resolution of the \SOne beam counter is measured using the UToF DAQ and found to be around \SI{30}{\pico\second}.
...
\begin{equation}
  ...
  \label{eq:tAve}
\end{equation}
where the $t_{\text{PMT}n}$ refer to the hit time of four PMTs used to read out \SOne.
...
\STwo consists of a single piece of plastic scintillator, which measures \SI{120 x 120 x 5}{\milli\metre} and is read out, via a light guide, by a 2'' Hamamatsu Photonics R1309 PMT~\cite{hamamatsu}.
...
EJ-200 plastic scintillator has a brightness of \num{10000}~photons/MeV deposited.
...
\SThree triggers are formed in the following manner, shown graphically in \citefig{fig:s3Trigger}, and are intended to identify the presence of a beam particle travelling between \SOne and \SThree.
...
An image of the HPTPC, displaying a cross-sectional view of the active volume, is shown in \citefig{fig:hptpc}.
...
In a secondary beam line (where the beam is produced by impinging some initial beam on a target) such as the T10 beam line at CERN, charged particles are selected by momentum.
...
\citefig{fig:tofVsMom} left and right, show the predicted time of flight for various particles as a function of particle momentum for the $\SOne - \SThree$ and  $\STwo - \SFour$ distances respectively.
...
\end{equation}
For \citefig{fig:s3MassDistribution} $p$ has been set to \SI{0.8}{\giga\electronvolt\per\clight} and $x_{\SThree} - x_{\SOne}$ is equal to the distance between \SOne and \SThree.
...
\citefig{fig:energyLoss} shows the mean charged particle energy loss for a variety of different materials as a function of particle $\beta \gamma$.
...
\citefig{fig:tvsa} shows the relationship between $t_{\SThree} - t_{\SOne}$ and signal amplitude (here labelled as A1) for one of the SiPMs in \SThree along with timing and signal cuts used to select particle types.
...
\citefig{fig:tvsa:4blocks} shows the reasoning for requiring both a signal amplitude and timing cut in order to select protons.
...
The variation in $p0$ and $p1$ between the different datasets may derive from the different fluxes experienced by \SOne in each case.
...
\begin{figure}[h]
  ...
  \end{adjustbox}
  \caption[Time of flight spectra measured in \SFour]{Time of flight spectra for the various moderator block configurations measured in \SFour. Here, the spectrum has been corrected for the bar efficiencies (see \citesec{sec:hptpc_beam_flux:methods:s4:efficiency}) and a flat background has been subtracted (see \citesec{sec:hptpc_beam_flux:methods:s4:bkg}).}
  \label{fig:s4Tof}
\end{figure}
...
The variation between the samples results these measurements having been performed at different times in the beam test.
At these different times, different PMT voltages may have been used, resulting in varying gains and thus changing efficiencies.
...
An example of a time of flight distribution with the combined signal and background functions fitted is shown in \citefig{fig:bkgSub}.
These backgrounds are subtracted from the proton counts presented in \citesec{sec:hptpc_beam_flux:results:s4}.
...
Beam flux measurements made with the \SThree detector are detailed in \citesec{sec:hptpc_beam_flux:results:s3} while those made with the \SFour detector are detailed in \citesec{sec:hptpc_beam_flux:results:s4}.
A comparison of the data with a Monte Carlo simulation is shown in \citesec{sec:hptpc_beam_flux:results:MCData}.
...
\citefigL{fig:protonKeData}, shows the kinetic energies of protons calculated using the \SOne to \SThree time of flight for varying numbers of moderator blocks.
...
\citefigR{fig:protonKeData}, shows the subset of those protons that are determined to impinge upon the TPC by the angle at which they are travelling through \SThree (see \citetab{tab:angDistS1} for the relevant angles for the TPC).
...
This is the desired energy regime where various neutrino interaction generators become discrepant (see \citefig{fig:generatorProtons}).
...
Additionally, \citefigR{fig:protonKeData}, shows that the addition of moderator blocks also has a substantial effect on the multiplicity of protons striking on the TPC.
...
\citefigL{fig:protonMipS3}, shows the number of MIPs detected in \SThree as a function of number of moderator blocks and off-axis angle $\theta$.
Similarly, \citefigR{fig:protonMipS3}, shows the same distribution but for protons.
...
\citefigL{fig:s3Ratios}, shows the ratio $N_{\text{proton}}/N_{\text{MIP}}$ as a function of $\theta$ and the number of moderator blocks.
...
A similar pattern is seen in \citefigR{fig:s3Ratios}, with the peak gradually shifting to greater off-axis angles with increasing numbers of moderator blocks being added.
...
\citefigL{fig:s4Rates}, shows that the number of MIPs per spill peaks between \ang{-2} and \ang{-1} for all the various samples.
A similar pattern is observed for protons in \citefigR{fig:s4Rates}, where the peak occurs at approximately \ang{-2}.
...
\citefigL{fig:s4Rates}, shows that MIP multiplicity changes significantly with the addition of moderator blocks from \num{236(4)} per spill for the unmoderated beam to \num{1132(9)} for 3 block configuration.
...
A similar pattern is observed for protons in \citefigR{fig:s4Rates}.
...
\citefigR{fig:protonKeData}, provides a measurement of the number of protons incident upon the TPC.
...
Significant variation is observed in the value of this systematic as the number of moderator blocks is changed.
Specifically, the value of this systematic is significantly higher for the 3 block sample.
This is thought to be due to the energy of most of the protons reaching the HPTPC vessel, which is just above the threshold required to reach \SFour. 
Therefore small changes, such as additional material in the beam line, lead to large changes in $n_{\SFour, \text{MC}}$, compared with the other samples where the proton energies are well above this threshold.
...
The uncertainty for the 0 block sample is significantly lower than for the other samples.
This is due to the much smaller variation in $(\SOne\STwo)_{\text{DToF}}$ between spills (see \citefig{fig:s1s2DeadtimeComp}), resulting in smaller variations in event weights and thus a lower dependence on the dead time correction.
...
From these tests an absolute efficiency of 0.8 was determined (this is the origin of the additional weighting of 1.25 detailed in \citesec{sec:hptpc_beam_flux:methods:s4:efficiency}).
...
This uncertainty has the greatest effect in the 4 moderator block case which is due to the very small number of protons detected relative to the background.

The systematic uncertainties are tabulated in \citetab{tab:systs}.
...
One can see that there is some agreement between the data and MC for the 1, 2, 3 and 4 blocks samples which indicates both that the corrections detailed in \citesec{sec:hptpc_beam_flux:methods} are well-motivated and that the simulation represents the data well in these cases.
In the case of the unmoderated beam, the data agrees less well with the simulation, both in the results shown in \citetab{tab:s3s4Ratios} and in the comparison between the simulated and measured time of flight profiles.
...
However, it is noted that this increase in the flux of low energy protons comes at the cost of reducing the proton/MIP measured in both \SThree and \SFour (to values of \num{0.03} and \num{0.003} for the full moderator respectively).
This could be an issue for a detector which had difficulties in distinguishing protons and MIPs since the overwhelming flux of MIPs would necessitate effective background suppression.
