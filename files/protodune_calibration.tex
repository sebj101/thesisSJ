...
This effect and its corrections are detailed in \citesec{sec:pdune_calibration:lifetime:data:sce}.
...
A theoretical analysis of these effects is given in Ref.~\cite{spaceCharge2021} and is summarised here.
The density of positive ions in the detector, $\rho^{+}$, satisfies the continuity equation,
\begin{equation}
	\frac{\partial \rho^{+}}{\partial t} + \div \left( \rho^{+} \vb{v^{+}} \right) = K \, ,
\end{equation}
where $\vb{v^{+}}$ is the drift velocity of the positive ions.
In the case of a surface detector such as ProtoDUNE-SP there is a constant and uniform charge injection rate, $K$, from cosmic rays (if one averages over a suitably long enough time period).
This assumption leads to the steady state case, where $\partial \rho^{+} / \partial t = 0$.
