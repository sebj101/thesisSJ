\chapter{Energy scale calibration of the ProtoDUNE-SP detector}
\label{sec:pdune_calibration}

Previously in \citesec{sec:dune_lbl:systs:det}, a summary of the detector systematics used in the DUNE long-baseline analysis is given.
A number of these detector systematics relate to the uncertainties in the measured energies of various particle types.
These uncertainties may result from various sources including mismodelling of the electric field, unaccounted for detector non-uniformities and mismeasurement of the electron lifetime.
The ProtoDUNE-SP detector, as a prototype of a single-phase DUNE FD module, provides an excellent opportunity to validate some of these systematics.
In particular, \citesec{sec:pdune_calibration:lifetime} provides a demonstration of a method for measuring the electron lifetime of the ProtoDUNE-SP detector using cosmic rays.
These measurements are then used to determine an effective bias on the reconstructed particle energy scale.
This bias is applied to FD simulation in \citesec{sec:pdune_calibration:osc} in order to determine the effect this has on DUNE's ability to measure key neutrino oscillation parameters.

The space charge corrections described and used in \citesec{sec:pdune_calibration:lifetime:sce} were developed by other members of the ProtoDUNE-SP collaboration.
...
Both of these measurements include a correction for the electron lifetime depending on the position of the hit in the detector, making it an important variable to measure.

The charge deposition per unit length is commonly termed \dqdx.
Typically, this \dqdx is measured in analogue-to-digital converter (ADC) units of \si{\ADC\per\cm} where this ADC unit is a measure of the signal amplitude on a wire.
...
Typically, charged particles generate either tracks or electromagnetic showers when traversing a LArTPC. 

High energy electrons and photons typically generate electromagnetic `showers' when they strike a high density material such as liquid argon.
...
These tracks have an energy deposition that is well-defined and relatively constant along their length.
...
\begin{figure}[h]
	...
	\end{minipage}
	\caption[Images of typical track and shower structures in ProtoDUNE-SP.]{Left: Image of a \SI{0.5}{\GeV\per\clight} electron candidate in ProtoDUNE-SP, showing typical branching shower structure, from~\cite{protodunePerformance}. Right: Image of a \SI{1}{\GeV\per\clight} stopping proton candidate in ProtoDUNE-SP, showing typical track structure, from~\cite{protodunePerformance}.}
...
The average energy loss is calculated here using the `Bethe-Bloch' formula~\cite{pdg2018} which gives the energy loss for a particle of charge $z$, mass $M$ and with a speed $v$ such that $\beta = v / c$ and $\gamma = 1 / \sqrt{1 - \beta^{2}}$:
\begin{equation}
...
$m_{e}$ is the electron rest mass and $K$ is a constant equal to \SI{0.307075}{\MeV\per\mole\square\cm}.
...
%% Track selection
From \citefig{fig:muonEDep} left and right, one can see that, aside from the last 40 or \SI{50}{\cm} of the track, muons have uniform energy deposition in liquid argon.
...
This reconstruction chain focusses on reconstructing track-like particles and has been shown to work well in the MicroBooNE experiment~\cite{pandoraUboone}. 
...
The difference around $\thetai{XZ} = \pm \ang{90}$ is caused by tracks travelling close to parallel with the drift direction while the difference at $\thetai{YZ} = \ang{-90}$ is due to tracks travelling vertically downwards (i.e. parallel to the wires of the collection plane). 
...
As a result of this, tracks for which $\ang{65} < \left| \theta_{XZ} \right| < \ang{115}$ or $\ang{-110} < \theta_{YZ} < \ang{-70}$ are excluded from the selection.
This selection is shown in \citefig{fig:dqdxAng}.
...
The 2 axes of the histogram are the corrected drift time, $t_{\text{drift}}$ and the \dqdx value for that particular track hit as measured by the collection plane. 
...
This is thought to be due to the finite electron lifetime.
...
A Landau distribution convolved with a gaussian~\cite{langaus} is fitted to each set of data points and is shown as a solid line in each case.
In both cases this function appears to match the data well within the chosen range.
...
\begin{figure}[h]
	...
	\end{adjustbox}
	\caption[\dqdx distributions for two different drift times in ProtoDUNE-SP]{\dqdx distributions for two different drift times in the same ProtoDUNE-SP data run. The solid lines show the result of fitting a Landau distribution convolved with a Gaussian to each set of points. Both sets of points are area normalised to 1.}
	\label{fig:dqdx1d}
\end{figure}
...
One can see that fit range used is $\SI{1}{\ms} < t_{\text{drift}} < \SI{2.5}{\ms}$ since in this range the extracted lifetime appears to behave as expected.
...
Therefore it seems that this method provides a viable way of extracting a value for the electron lifetime.
...
\subsection{Space charge effects (SCE)}
...
In fact, the ICARUS T600 collaboration reports an argon ion mobility of \SI{0.9d-3}{\cm\squared\per\volt\per\second}~\cite{icarusSpaceCharge} compared with an electron mobility of around \SI{470}{\cm\squared\per\volt\per\second}~\cite{electronMobility}.
...
This build up of ions is referred to as space charge and an analysis of these effects is given in Ref.~\cite{spaceCharge2021}.
...
In all cases, a function of the form shown in \citeeq{eq:lifetimeFromdqdx} is fitted to the data points.
The fit variables $A$ and $\tau$ correspond to the same variables in \citeeq{eq:lifetimeFromdqdx}.
$A$ is the value of \dqdxMPV in the case where no drift time has elapsed. 
One can observe that the fit range is between \tdrift values of \SI{0.2}{\ms} and \SI{1.5}{\ms}.

The choice is made to exclude lower values of \tdrift due to the behaviour of the peak values in this region. 
For all the samples the values of \dqdxMPV seem to be below those in the neighbouring time segments.
These hits occur very close to the wires and thus may be especially vulnerable to having their positions and calculated \dqdx values distorted by space charge effects.
...
Therefore, SCE which have not been fully corrected for dominate the shape of the distribution at longer drift times.
This is thought to be the cause of the measured values of \dqdxMPV increasing with drift time at larger values of \tdrift.

Each of these runs also has associated lifetimes measured by each of the purity monitors.
\citefig{fig:lifetimeComp} shows a comparison of the lifetime measured with the method above with the lifetime measured by the uppermost of the purity monitors (see \citesec{sec:protodune:prms} for further details).
The lifetimes measured by the other two purity monitors are not displayed because they both have larger associated errors and their central values fall within the errors provided by the upper purity monitor.
...
The reason for these larger discrepancies occurring at longer lifetimes results from the very small changes in \dqdxMPV that must be measured in order to extract the lifetime and the consequent sensitivity to SCE.
For a purity monitors, the electric field may be changed to increase the drift time and thus the sensitivity to longer lifetimes. 
However, for a large TPC, changing the electric field regularly is typically not an option.
On the other hand, using tracks has the advantage of being able to measure the electron lifetime in different areas of the detector, something that is not possible with a purity monitor in a fixed position.

As mentioned previously, obtaining the correct value of the electron lifetime is crucial when performing calorimetric measurements of particle energy. 
...
It is demonstrated above that, in the ProtoDUNE-SP detector, discrepancies are observed between measured electron lifetimes from two different sources. 
...
To assess the effect a bias in the electron lifetime could have on an oscillation analysis, the CAFAna framework (see \citesec{sec:dune_lbl:sensitivities:cafana}) is used together with simulated FD events.
...
\begin{figure}[h]
	...
	\begin{subfigure}[t]{.5\textwidth}
		\begin{adjustbox}{max totalsize=\textwidth, center}
			\input{files/figures/protodune_calibration/asimovNumuRhc_ratio}
		\end{adjustbox}
	...
	\begin{subfigure}[t]{.5\textwidth}
		\begin{adjustbox}{max totalsize=\textwidth, center}
			\input{files/figures/protodune_calibration/asimovNueRhc_ratio}
		\end{adjustbox}
...
This indicates that some combination of both the systematics and oscillation parameters is able to account for the introduced bias.
In an experiment where we are attempting to measure the true value of the oscillation parameters, this is undesirable.
Introducing a further systematic of a similar form to the bias could prevent this, but at the cost of reduced sensitivity. 
...
In this case, statistical fluctuations are introduced to the data, equivalent to those that would be expected for 1 year's POT.
Additionally, the oscillation parameters \thetai{13}, \thetai{23}, \deltami{32}, \dcp and $\rho$ are all thrown within the limits shown in \citetab{tab:paramThrows}.
...
In this case, the mean bias in the measured value of \deltami{32} has now increased to \SI{1.39(4)d-5}{\eV\squared}.
This equates to a mismeasurement of approximately 0.6\% in the value of \deltami{32}.
This is still small when compared to the expected resolution of \deltami{32}.  
...
We find that this energy scale bias has a minimal effect on the measured oscillation parameters, other than biasing the measured value of \deltami{32} by 0.2\%.
...
Additionally, the bias in the measured value of \deltami{32} is increased to 0.6\%.
