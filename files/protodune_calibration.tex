...
The average energy loss is calculated here using the `Bethe-Bloch' formula~\cite{pdg2018} which gives the energy energy loss for a particle of charge $z$, mass $M$ and with a velocity $v$ such that $\beta = v / c$ and $\gamma = 1 / \sqrt{1 - \beta^{2}}$:
\begin{equation}
...
These downward going tracks travel parallel to the collection plane where \dqdx is measured, meaning that most of the charge is deposited on a single wire. 
This makes it difficult to determine the point at which each charge deposition occurs and thus leads to poorly reconstructed values of \dqdx.
