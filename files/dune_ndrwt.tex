...
However, only the convolution of the neutrino flux, cross-section and detector effects is actually measured (see \citeeq{eq:eventRateConv}).
Neutrino oscillation experiments therefore require the assistance of models to successfully constrain the oscillation parameters.
...
Its large mass and the intensity of the DUNE beam will allow it to collect a large number of neutrino interactions.
...
Additionally the dense medium may mean that low energy particles are not reconstructed.
...
CAFAna was originally developed the the NOvA oscillation analysis but has since been ported for used by both the DUNE and Short-Baseline Neutrino (SBN) collaborations~\cite{cafana}.
...
For all of these fits, the full set of flux, cross-section and detector systematics are set to their nominal values while the oscillation parameters are set to their NuFit~4.0~\cite{nufit4} values.
...
The ability to evaluate these models as a function of final state will be useful since this extra degree of freedom should allow some over-constraining of the interaction models.
...
However, there are also limitations to the method which are detailed in the relevant section. 
