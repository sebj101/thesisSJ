...
For pions in the energy range $\SI{0.4}{\GeV\per\clight} < p < \SI{1.0}{\GeV\per\clight}$ the
...
Here one can see that, for all final states, the correct final state is reconstructed at least 80\% of the time.
...
These ratios are shown in \citefigL{fig:Q2CompFhc} and \citefigL{Q2CompRhc}. 
...
Additionally, \citefigR{fig:Q2CompFhc} and \citefigR{Q2CompRhc} show the equivalent distributions for true selections and variables. 
...
\begin{figure}[h]
	...
	\caption[Final state pion multiplicity confusion matrix from ND-LAr simulations]{Final state pion multiplicity confusion matrix from ND-LAr simulations, from~\cite{ndCdr}.}
...
\begin{figure}[h]
	...
	\caption[Ratio of $Q^{2}_{\textrm{reco}}$ between GENIE and NuWro mock data in ND-LAr]{Ratio of $Q^{2}_{\textrm{reco}}$ between GENIE and NuWro mock data in ND-LAr, from~\cite{ndCdr}.}
	\label{fig:q2LarFhc}	
\end{figure}
