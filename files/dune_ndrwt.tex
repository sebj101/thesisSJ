...
\begin{figure}[h]
	...
	\begin{minipage}[t]{.5\linewidth}
		\begin{adjustbox}{max totalsize=\linewidth, center}
			\input{files/figures/dune_ndrwt/energyByModeFhc_unstacked}	
		\end{adjustbox}
	\end{minipage}
	\hfill
	\begin{minipage}[t]{.5\linewidth}
		\begin{adjustbox}{max totalsize=\linewidth, center}
			\input{files/figures/dune_ndrwt/energyByModeRhc_unstacked}	
		\end{adjustbox}
	\end{minipage}
...
\citesec{sec:dune_ndrwt:bdt} provides a description of a tool, developed by other members of the DUNE collaboration, that can be used to easily reweight an existing sample of MC events to resemble those of an alternative neutrino interaction generator.
...
Further details of the general method are available in Ref.~\cite{reweightingWithBDTs}.
...
This BDT approach has been validated and used elsewhere in the DUNE collaboration, notably to produce a sample of mock data with the energies of final state protons arbitrarily changed~\cite{ndCdr}.
...
The remaining constants in the first terms are derived in Ref.~\cite{gluckstern} while those in the second term result from the Highland formula~\cite{highland}.
...
\citefig{fig:interactionSpline} shows the probability for a pion to not interact in the ECAL at various momenta with a cubic spline plotted through the points.
...
The term `ranging out' here refers to a particle losing all its kinetic energy in the detector without scattering inelastically.
...
Measurements of $Q^{2}$ are also made in a simulation of the liquid argon component of the DUNE near detector, ND-LAr (see \citesec{sec:dune:nd:lar} for details of the detector).
...
One can see that the bias in \dcp varies between being negligible up to approximately $0.19\pi$ (\ang{32}). 
The large jump at $\dcpTrue \approx 0.55\pi$ coincides with \thetai{13} and \thetai{23} simultaneously being pulled away from their inputted true values, presumably to compensate for the large \dcp bias.
...
Those events with high $E_{\text{vis, reco}}$ or $p_{\text{vis, reco}}$ but a low value for the same true variable are caused by those events where electromagnetic or neutron activity is misidentified as being produced by charged hadrons, erroneously increasing the value of $E_{\text{vis, reco}}$ and $p_{\text{vis, reco}}$.
...
Even with this reweighting, the fit still fails due to a lack of freedom from the systematic uncertainties.
This is due to the very high statistics of the ND samples used in the oscillation analysis, which lead to tight constraints on many of the systematics.
As a result, no combination of these constrained systematics and the oscillation parameters can reproduce the data.
