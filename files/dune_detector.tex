...
As discussed in \citesec{sec:theory:currentState}, several questions remain in the field of neutrino oscillations.
In particular, the ordering of the neutrino mass eigenstates is not yet known and it is also unknown if the CP-violating phase of the PMNS matrix exist.
If it is proven that $\dcp \neq 0, \pi$, this would be proof of the existence of CP-violation in the lepton sector.
DUNE aims to shed light on both of these issues.
Additionally, DUNE aims to be able to resolve the octant of \thetai{23} (i.e. is $\thetai{23} > \pi/4$ or $< \pi/4$?) if it is non-maximal and usher in a new era of precision measurements of the PMNS matrix. 

Due to the large active mass of the far detector, DUNE is also designed to observe neutrinos from a galactic core-collapse supernova if one should occur during the running time of the experiment.
...
The running times are given in terms of staged years. 
...
This allows improvements such as a larger drift length, resulting in a reduction in dead material.
...
In order to measure the \dcp, a neutrino experiment must accurately measure (amongst others) the probability of a muon neutrino oscillating into an electron neutrino, $P_{\numu\rightarrow\nue} (E_{\nu})$, where
\begin{equation}
...
The number of events per energy spectrum (as is measured in an experiment) can be expressed for an ideal detector as
\begin{equation}
...
These pixels have a spacing of \SI{4}{\milli\metre} and the time-binning of the signals is expected to be \SI{2.5}{\micro\second}.
...
A diagram of the ECAL is shown in \citefig{fig:ecalDiag}.
...
As mentioned previously, significant disagreements exist between the predictions made by different neutrino interaction generators.
This problem arises due to an incomplete knowledge of neutrino interactions with nuclei.
In order for DUNE to successfully reach its predicted sensitivities, systematic uncertainties due to neutrino interaction modelling will have to be controlled.

DUNE plans to reduce its dependence on its neutrino interaction model and detector modelling by employing movable ND components to sample different neutrino beam fluxes.
...
This will allow the deconvolution of the neutrino interaction model and flux. 
Additionally, these additional samples can be combined in linear combinations to produce ND fluxes which are very similar to expected oscillated flux.
...
ND-LAr and ND-GAr can move to off-axis positions in order to sample different fluxes while SAND remains in the same position to continue monitoring the beam.
A diagram of the ND with ND-LAr and ND-GAr in their maximal off-axis positions (\SI{30.5}{\metre} off-axis) is shown in \citefig{fig:ndDiagOffAxis}.
This distance corresponds to an off-axis angle of \ang{3.2}.
The detectors will also make intermediate stops between the maximum off-axis and on-axis positions so as to sample the various fluxes.

\begin{figure}[h]
  \centering
  \includegraphics[width=.7\linewidth]{files/figures/dune_detector/ndDiagOffAxis}
  \caption[Diagram of DUNE ND in off-axis configuration]{Diagram of DUNE ND in the off-axis configuration from~\cite{ndCdr}.}
  \label{fig:ndDiagOffAxis}
\end{figure}
...
The DUNE neutrino beam also has this property and several different off-axis \numu fluxes are shown in \citefig{fig:offAxisFluxes}, both as a function of off-axis angle and as a function of off-axis detector distance.
...
This sampling of various fluxes will allow problems with the neutrino interaction model to be more easily diagnosed since the various off-axis samples will over-constrain the interaction model.
For example, for a neutrino interaction model to be judged as correct, its predictions will have to match the data across a range of different energies rather than just at one flux.
...
The different off-axis samples can be linearly combined to match an oscillated FD neutrino energy spectrum.
...
An example of this FD `flux matching' can be seen in \citefig{fig:dunePrismFluxMatch}.

\begin{figure}[h]
  \centering
  \includegraphics[width=.8\linewidth]{files/figures/dune_detector/dunePrismFluxMatch}
  \caption[Example of DUNE-PRISM FD flux matching]{An example of the FD flux matching performed with the DUNE-PRISM technique, from~\cite{ndCdr}. The FD \numu flux is shown as a black line while the linear combination of ND off-axis fluxes is shown in blue.}
  \label{fig:dunePrismFluxMatch}
\end{figure}
...
Given that DUNE-PRISM relies on the well known relationship between off-axis angle and neutrino energy, it is important that the beam remains stable while off-axis data is collected.

To this end, the DUNE ND must feature a detector which remains on axis (where sensitivity to changes in the beam is at its maximum) which can rapidly detect changes in the beam.
It must also measure the changes as a function of a neutrino energy rather than just as an interaction rate.
The detector that fulfills this role is SAND.

Furthermore, SAND will feature a non-Argon target material.
This will allow interactions on multiple different nuclei with the same neutrino flux to be measured.
This may help to constrain systematic uncertainties arising from the neutrino interaction model.

A cut-away diagram of the reference design of SAND is shown in \citefig{fig:sandDiag}.
In this configuration, the primary target is a 3D scintillator tracker (3DST).
...
This magnet produces a field of \SI{0.6}{\tesla}.

\begin{figure}[h]
  \centering
  \includegraphics[width=.55\linewidth]{files/figures/dune_detector/sandDiag}
  \caption[Cut-away diagram of SAND]{Cut-away diagram of SAND from~\cite{ndCdr} showing the various components. The central 3DST is shown in light green, the low-density tracker is shown in magneta, the ECAL is shown in green, the magnet coil is shown in gold and the return yoke is shown in grey.}
  \label{fig:sandDiag}
\end{figure}

The 3DST is an active target made up on numerous $\SI{1}{\cm} \times \SI{1}{\cm} \times \SI{1}{\cm}$ plastic scintillator cubes which are each read out via three orthogonal wavelength-shifting fibres.
A diagram of 8 of these cubes together with the fibres are shown in \citefig{fig:3dst}.
The complete 3DST will contain \num{10.3d6} scintillator cubes giving a total active mass of \SI{11.8}{\tonne}.
These 3-dimensional signals allow neutrino interaction vertices to be precisely and accurately reconstructed to check for any movement of the beam centre.

\begin{figure}[h]
  \centering
  \includegraphics[width=.55\linewidth]{files/figures/dune_detector/3dst}
  \caption[Assembly of 8 cubes as used in the 3DST]{An assembly of 8 plastic cubes together with wavelength shifting fibres as used in the 3DST. Taken from~\cite{ndCdr}.}
  \label{fig:3dst}
\end{figure}

The low density trackers exist to reconstruct those charged particles that escape the 3DST (for example, muons produced in neutrino interactions).
The curvature of these tracks in the magnetic field can be then be used to reconstruct the particle momentum, and with the correct particle identification, the particle's energy.

The KLOE ECAL is made up of alternating layers of \SI{0.5}{\mm} lead foil and \SI{1}{\mm} diameter scintillating fibres arranged into modules.
These modules are arranged into a cylindrical barrel and two endcaps.
A picture of the KLOE ECAL is shown in \citefig{fig:kloeEcal} with the endcap and barrel modules outlined. 
The barrel is \SI{4.3}{\metre} long with a thickness of \SI{23}{\cm}, while the endcaps have a diameter of \SI{3.9}{\metre} and a thickness of \SI{23}{\cm}.
The total weight of the ECAL is approximately \SI{100}{\tonne}.
The KLOE superconducting magnet measures \SI{4.8}{\metre} in diameter and \SI{4.3}{\metre} in length and generates a field of \SI{0.6}{\tesla}.

\begin{figure}[h]
  \centering
  \includegraphics[width=.6\linewidth]{files/figures/dune_detector/kloeEcal}
  \caption[Image of the KLOE ECAL]{Image of the KLOE calorimeter from~\cite{ndCdr}. An endcap and a barrel module are outlined.}
  \label{fig:kloeEcal}
\end{figure}
