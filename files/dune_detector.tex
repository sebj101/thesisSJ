...
DUNE plans to reduce its dependence on the neutrino interaction model and detector modelling by employing movable ND components to sample different neutrino fluxes.
...
Additionally, these extra samples can be combined in linear combinations to produce ND fluxes which are very similar to expected oscillated flux.
...
To this end, the DUNE ND must feature a detector which remains on axis (where sensitivity to changes in the beam is at its maximum) which can rapidly detect changes in the neutrino flux.
