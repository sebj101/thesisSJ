...
\begin{align}
  ...
  \end{pmatrix}
\end{align}
where $s_{ij} = \sthetai{ij}$, $c_{ij} = \cthetai{ij}$, $\thetai{ij} = [0, \pi/2]$ and $\dcp = [0, 2\pi]$~\cite{pmnsMatrix}.
...
To understand neutrino oscillations we begin by consider the case of a neutrino propagating freely at its point of production ($x=t=0$) as a pure flavour state.
...
However, the sign of \deltam{21} is known.
...
Using the fact that $|\deltami{21}/\deltami{31}| \approx 0.03$ and the fact that \thetai{13} is also small the following approximations can be used~\cite{Nunokawa_2008} for long baseline oscillations:
\begin{align}
