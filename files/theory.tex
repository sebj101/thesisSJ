...
As shown in \citeeq{eq:massFlavSuper}, the flavour state is a superposition of mass eigenstates.
...
This means that \citeeq{eq:nuPropagation} can be expressed as
\begin{align}
...
\end{align}
Using this and \citeeq{eq:oscillationProb}, we find that
\begin{equation}
...
We can now redefine $H_{M}$ in terms of an `effective' mass splitting and mixing angle (\deltam{m}{} and $\theta_{m}$ respectively) so that it becomes
\begin{equation}
...
\begin{itemize}
\item The mixing angles of the PMNS matrix, \thetai{12}, \thetai{13}, \theta{23}
\item The mass splittings, \deltam{2}{1}, \deltam{3}{1}
\item The CP-violating phase of the PMNS matrix, \dcp
\end{itemize}
where \deltam{2}{1} and \deltam{3}{1} is one way to parametrise the differences between the 3 different mass states.
...
\end{align}
%One can see that \citeeq{eq:numuSurvival} depends upon \dcp which will have the opposite sign for antineutrinos.
%However, given that 
...
Using the fact that $\deltam{2}{1}/\deltam{3}{1} \approx 0.03$ and the fact that \thetaij{1}{3} is also small the following approximations can be used~\cite{Nunokawa_2008} for long baseline oscillations:
\begin{align}
...
The detected \nue spectra is dominated by neutrinos produced by $^{8}\text{B}$ decays as shown in \citefig{fig:solarNuFluxes}.
...
Energies for long baseline experiments are \order{\SI{1}{\giga\electronvolt}}, which given the value of \deltami{31} (or \deltami{32} for inverted hierarchy) requires a baseline of a few \SI{100}{\kilo\metre} to observe oscillations.
...
\citefig{fig:theta23deltam} shows the current limits set by these experiments on \ssthetai{23} and \deltami{32}.
...
However, to attain a $5\sigma$ measurement of \dcp the next generation of oscillation experiments will be required.
