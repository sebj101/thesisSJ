...
\begin{figure}[h]
  ...
  \end{subfigure}
  \caption[Examples of diagrams for charged (CC) and neutral (NC) current neutrino interactions.]{Left: Example of a neutrino charged current interaction with a proton. Right: Example of a neutrino neutral current interaction with a proton.}
  \label{fig:nuIntEx}
\end{figure}
...
In order to evaluate this term, we must first consider the production of the neutrino and the muon.
One of the ways in which this may occur (and the process which we consider here) is the decay of a charged pion to the aforementioned particles.
We can then express the phase difference between two neutrino mass eigenstates $i$ and $j$ in the pion rest frame as
\begin{equation}
...
We consider the kinematics of this pion decay in the rest frame of the pion.
...
We can therefore write our neutrino propagator as
\begin{equation}
\exp(-i m_{i} \tau_{i}) = \exp(-i \left(m_{i}\right) \frac{L_{0}}{2E^{\nu}_{0}} )
\end{equation}
which will keep the relative phases the same.
We then insert this into the neutrino propagation amplitude of \citeeq{eq:oscAmp} to get
\begin{equation}
...
\begin{figure}[h]
  ...
  \begin{subfigure}[t]{0.49\textwidth}
    \begin{adjustbox}{max totalsize={\textwidth}, center}
      \input{files/figures/theory/twoNuFixedE}
    \end{adjustbox}
  ...
  \begin{subfigure}[t]{0.49\textwidth}
    \begin{adjustbox}{max totalsize={\textwidth}, center}
      \input{files/figures/theory/twoNuFixedL}
    \end{adjustbox}
...
\subsubsection{The Mikheyev-Smirnov-Wolfenstein effect}
...
As a result, electron neutrinos experience an additional potential, $V_{e} = \sqrt{2} G_{F} N_{e}$, where $G_{F}$ is the Fermi constant and $N_{e}$ is the electron density of the medium.
...
This allows us to redefine $H_{M}$ in terms of an `effective' mass splitting and mixing angle (\deltami{m} and $\theta_{m}$ respectively) so that it becomes
\begin{equation}
...
We can derive expressions for these `effective' mixing parameters and find that, for electron neutrinos,
\begin{align}
...
\begin{figure}[h]
  ...
  \begin{adjustbox}{max totalsize={.6\textwidth}, center}
    \input{files/figures/theory/twoNuMSWComp}
  \end{adjustbox}
...
\item The two independent mass splittings, \deltami{21}, \deltami{31}
...
While the sign of \deltami{21} is known, it is not clear whether $\nu_{3}$ is the heaviest or the lightest mass eigenstate.
...
\begin{figure}[h]
  ...
  \begin{subfigure}[t]{0.49\textwidth}
    \begin{adjustbox}{max totalsize={\textwidth}, center}
      \input{files/figures/theory/nueAppVacuum}
    \end{adjustbox}
...
\begin{figure}[h]
  ...
  \begin{subfigure}[t]{0.49\textwidth}
    \begin{adjustbox}{max totalsize={\textwidth}, center}
      \input{files/figures/theory/nueAppMatter}
    \end{adjustbox}
...
\citefig{fig:nufit5Contours} shows the confidence limits obtained for the various parameters.
...
Although the absolute values of $\deltami{21}$ and $\deltami{31}$ have $\pm1\sigma$ uncertainties of less than 3\% and 2\% respectively, the sign of $\deltami{31}$ is not known, i.e. it is not known whether $\nu_{3}$ is the lightest or the heaviest mass eigenstate.
...
Additionally, the Kamioka Liquid Scintillator Antineutrino Detector (KamLAND) experiment, located in Japan, was also able to constrain \thetai{12} and \deltami{21} using the flux of \anue from a number of nuclear reactors located at baselines of around \SI{100}{\km} in Japan.
...
\thetai{13} was first measured in 2012 using reactor neutrino experiments at baselines of $\order{\SI{1}{\kilo\metre}}$, however it is also possible to measure it using long-baseline \nue appearance in a beam of \numu.
...
Limits from the NOvA and T2K experiments are shown in \citefig{fig:t2kLimits}.
...
\begin{figure}[h]
	...
	\end{minipage}
	\caption[Limits on various neutrino oscillation parameters from the T2K and NOvA experiments]{Left: Confidence intervals for \dcp, \ssthetai{23} and \sstwothetai{13} as measured by the T2K experiment, from~\cite{t2kRecent}. Right: Two-dimensional contours for \dcp and \ssthetai{23} as measured by the NOvA experiment, from~\cite{novaRecent}. The upper panel shows the contours for the normal neutrino mass hierarchy while the lower panel shows the contours for the inverted neutrino mass hierarchy.}
    \label{fig:t2kLimits}
\end{figure}
