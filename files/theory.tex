...
However, each mass eigenstate propagates at a different rate due the differences in energy, $E_{j} = \sqrt{p_{j}^{2} + m{j}^{2}}$. 

Therefore, after a time $t$ has passed, the wavefunction becomes
\begin{equation}
  \ket{\nu_{\alpha}(t)} = \sum_{j=1}^{3} U^{*}_{\alpha j} e^{-iE_{j}t} \ket{\nu_{j}}
...
\end{equation}
which means that Eq.~\ref{eq:nuPropagation} can be expressed as
\begin{equation}
  \ket{\nu_{\alpha}(t)} = \sum_{j=1}^{3} U^{*}_{\alpha j} e^{-it \left( E+\frac{m_{j}}{2E}\right)} \ket{\nu_{j}} \, .
\end{equation}

The next step is to ask what is the probability that the state $\ket{\nu_{\alpha}(t)}$ will be detected as another flavour state, $\ket{\nu_{\beta}}$?
We find that
\begin{equation}
  P_{\nu_{\alpha} \rightarrow \nu_{\beta}} = \left|\braket{\nu_{\beta} | \nu_{\alpha}(t)} \right|^{2}
\end{equation}
